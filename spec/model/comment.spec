...
RSpec.describe Comment, type: :model do
  ...
  describe 'validations' do
    ...
    it 'should have a text' do
      comment = Comment.create(text: nil, post:, author: user)
      expect(comment).to_not be_valid

      comment = Comment.create(text: 'test', post:, author: user)
      expect(comment).to be_valid
    end

    # text must not exceed 100 characters.
    it 'should have a text with length less than 100' do
      comment = Comment.new(text: 'a' * 101, post:, author: user)
      expect(comment).to_not be_valid

      comment = Comment.new(text: 'a' * 99, post:, author: user)
      expect(comment).to be_valid
    end
  end
  ...
  describe 'associations' do
    ...
    it "should increment the post's comments_counter when a comment is created" do
      expect { Comment.create(text: 'test', post:, author: user) }.to change { post.comments_counter }.by(1)
    end
